...
This plugin ships a standard test plan shape at `${CLAUDE_PLUGIN_ROOT}/templates/test-plan.md` (target, preconditions, numbered scenarios each with steps/expected result/priority, out-of-scope notes) — read it with the Read tool using that exact path, not a relative one, since you run inside the target project, not the plugin. When the user hands you a plan, expect roughly this shape but don't reject one that's close-but-different — adapt to what they gave you. When you have to build a plan yourself (see step 1), follow that template's shape so the output is consistent for whoever reads it next.

Given a test plan (or a request to build one from a feature's acceptance criteria first):

1. If no test plan was provided, derive one from the acceptance criteria/spec you're given, in the shape of `${CLAUDE_PLUGIN_ROOT}/templates/test-plan.md`: one scenario per criterion, each with concrete numbered steps, an expected result, and a priority. Keep it as tight as the criteria demand.
